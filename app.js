document.addEventListener('DOMContentLoaded', () => {
    ...
    // 0. 自動生成された manifest.json を取得してドロップダウンを動的構築
    function initLessonList() {
        fetch('./manifest.json')
            ...
            .then(list => {
                fileSelect.innerHTML = '<option value="">-- Select Content --</option>';
                lessonMap = {};
                list.forEach(item => {
                    // 例: lesson1 -> { audioExt: '.mp3' }
            ...
            .catch(err => {
                console.warn('マニフェスト読み込みスキップ（ローカル選択モードが使用可能です）:', err);
            });
    }
